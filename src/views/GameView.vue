...
const fetchGame = async () => {
  ...
  try {
    ...
    if (gameData) {
      ...
      if (gameData.status === GameStatus.FINISHED && gameData.isAnalyzed) {
        showAnalytics.value = true
      }
    } else {
    ...
    <DashboardLayout>
      ...
      <div v-else class="flex flex-col pb-12 h-full min-h-[calc(100vh-120px)] overflow-x-hidden px-4">
        <div 
          ...
          :class="game.status === GameStatus.FINISHED
            ? 'lg:grid-cols-[120px_1fr_800px]' 
            : 'lg:grid-cols-[120px_1fr] lg:max-w-7xl mx-auto'"
        >
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    class="w-full py-5 bg-indigo-600 text-white text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-indigo-500 transition-all shadow-2xl shadow-indigo-500/20 active:scale-95"
                  >
                    Request AI Analysis
                  </button>
                </div>
          ...
          <div v-if="game.status === GameStatus.FINISHED && game" class="lg:hidden mt-8 px-4 pb-12 relative">
            ...
            >
              ...
              >
                ...
                <button
                  @click="triggerAnalysis"
                  class="px-10 py-4 bg-indigo-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-500 transition-all shadow-2xl shadow-indigo-500/20 active:scale-95"
                >
                  Request Analysis
                </button>
              </div>
