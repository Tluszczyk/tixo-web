    ...
    if (allGames.length > 0) {
      ...
      if (featured) {
        ...
        const activeGames = allGames.filter(
          (g) =>
            g.status === GameStatus.IN_PROGRESS || g.status === GameStatus.WAITING_FOR_OPPONENT,
        )
    ...
    <DashboardLayout>
      <template #header-left>
        <div class="flex items-center gap-4">
          <div class="text-2xl font-black tracking-tighter uppercase italic">
            Browser<span class="text-indigo-500">.</span>
          </div>
        </div>
      </template>
      ...
      <div class="space-y-20 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div class="space-y-8">
            <div
              class="inline-flex items-center gap-3 px-4 py-1.5 rounded-full glass border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-[0.2em]"
            >
          ...
          <div class="flex justify-center lg:justify-end w-full">
            ...
            >
              <!-- Decorative Glow -->
              <div
                class="absolute inset-0 bg-indigo-500/10 blur-[100px] rounded-full group-hover:bg-indigo-500/20 transition-all duration-1000"
              ></div>
