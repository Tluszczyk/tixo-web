...
const emit = defineEmits(['turn-select', 'trigger-analysis'])
...
const summary = computed(() => {
  ...
  // If only one move was played, shifts will be empty. Handle initial state if needed.
  if (shifts.length === 0 && probs.length > 0 && probs[0] !== undefined) {
     shifts.push({
       turn: 1,
...
<template>
  <div class="flex flex-col h-full">
    ...
    >
      <!-- Top Banner: Match Result (Always Visible) -->
      <div
      ...
      >
        ...
        <BaseAnalyticsChart 
          v-else-if="analytics && analytics.winProbabilityX && analytics.winProbabilityX.length > 0"
          :option="evalOption" 
          :selected-turn="selectedTurn"
          @turn-select="emit('turn-select', $event)" 
        />
        <div v-else class="flex flex-col items-center gap-4 opacity-20">
           <i class="pi pi-chart-line text-4xl"></i>
           <span class="text-[10px] font-black uppercase tracking-widest">No evaluation data</span>
        </div>
      </div>

      <!-- Temporal Delta Chart -->
      <div
        v-if="analytics && analytics.moveTimestamps && analytics.moveTimestamps.length > 1"
      ...
      <!-- Tactical Breakdown -->
      <div v-if="analysisStatus === 'ANALYSIS_COMPLETE' || analysisStatus === 'ANALYSIS_IN_PROCESS'" 
           class="glass border-glass-border rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 flex flex-col shadow-2xl min-h-[300px] justify-center">
        
        <div v-if="analysisStatus !== 'ANALYSIS_COMPLETE' && (!analytics || !analytics.moveQualities || analytics.moveQualities.length === 0)" 
             class="flex flex-col items-center gap-4 py-12">
          <i class="pi pi-spin pi-spinner text-4xl text-indigo-500"></i>
          <span class="text-[10px] font-black uppercase tracking-widest text-app-text-muted opacity-40">Classifying Move Quality...</span>
        </div>

        <template v-else-if="summary && analytics && analytics.moveQualities && analytics.moveQualities.length > 0">
          <h4
        ...
        <div v-else class="flex flex-col items-center gap-4 opacity-20 py-12">
           <i class="pi pi-chart-bar text-4xl"></i>
           <span class="text-[10px] font-black uppercase tracking-widest">No tactical data</span>
        </div>
      </div>
